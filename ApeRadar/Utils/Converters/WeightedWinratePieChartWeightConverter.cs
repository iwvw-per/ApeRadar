...
namespace ApeRadar.Utils.Converters
{
    internal class WeightedWinratePieChartWeightConverter : IMultiValueConverter
    {
        public object Convert(object[] value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            SKColor labelColor = ThemeManager.GetSkColor("AppChartTextColor", SKColors.Black);
            if (Double.TryParse(value[0] as string, out double soloWeight) && Double.TryParse(value[1] as string, out double div2Weight) && Double.TryParse(value[2] as string, out double div3Weight) && Double.TryParse(value[3] as string, out double shipMaxWeight))
            {
                return new ISeries[]
                {
                    new PieSeries<double>
                    {
                        ...
                        DataLabelsSize = 10,
                        DataLabelsPaint = CreateDataLabelPaint(labelColor, fontFamily),
        ...
        private static SolidColorPaint CreateDataLabelPaint(SKColor color, string? fontFamily)
        {
            return new SolidColorPaint
            {
                Color = color,
                SKTypeface = string.IsNullOrWhiteSpace(fontFamily) ? null : SKTypeface.FromFamilyName(fontFamily)
            };
        }
    }
}
